...
_Column.prototype.updateDefinition = function (updates) {
  return new Promise((resolve, reject) => {
    ...
    if (!this.isGroup) {
      ...
      this.table.columnManager.addColumn(definition, false, this).then(column => {
    ...
    if (rowIndex > -1) {
      ...
      if (!ifVisible) {
        if (_Tabulator.prototype.helpers.elVisible(rowEl)) {
          offset = _Tabulator.prototype.helpers.elOffset(rowEl).top - _Tabulator.prototype.helpers.elOffset(this.element).top;
      ...
      switch (this.renderMode) {
        case "classic":
          this.element.scrollTop = _Tabulator.prototype.helpers.elOffset(rowEl).top - _Tabulator.prototype.helpers.elOffset(this.element).top + this.element.scrollTop;
          break;
        case "virtual":
...
RowManager.prototype._genRemoteRequest = function () {
  ...
  table.modules.ajax.sendRequest().then(data => {
...
Row.prototype.updateData = function (updatedData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var attrname in updatedData) {
      var columns = this.table.columnManager.getColumnsByFieldRoot(attrname);
      columns.forEach(column => {
        var cell = this.getCell(column.getField());
        if (cell) {
...
Row.prototype.delete = function () {
  return new Promise((resolve, reject) => {
    ...
    if (this.table.options.history && this.table.modExists("history")) {
      ...
      this.table.modules.history.action("rowDelete", this, {
        data: this.getData(),
        pos: !index,
        index: index
      });
    ...
    if (data) {
      data.forEach(function (item) {
        ...
        if (row) {
          ...
          self.rowManager.addRows(item).then(newRows => {
            responses--;
...
_Tabulator.prototype.updateOrAddRow = function (index, data) {
  return new Promise((resolve, reject) => {
    ...
    if (row) {
      ...
      row = this.rowManager.addRows(data).then(rows => {
        //recalc column calculations if present
        if (this.modExists("columnCalcs")) {
          this.modules.columnCalcs.recalc(this.rowManager.activeRows);
        }
...
_Tabulator.prototype.scrollToRow = function (index, position, ifVisible) {
  return new Promise((resolve, reject) => {
    var row = this.rowManager.findRow(index);
    if (row) {
      this.rowManager.scrollToRow(row, position, ifVisible).then(() => {
        resolve();
      }).catch(err => {
        reject(err);
      });
...
_Tabulator.prototype.updateColumnDefinition = function (field, definition) {
  return new Promise((resolve, reject) => {
    var column = this.columnManager.findColumn(field);
    if (column) {
      column.updateDefinition(definition).then(col => {
        resolve(col);
      }).catch(err => {
        reject(err);
      });
...
_Tabulator.prototype.scrollToColumn = function (field, position, ifVisible) {
  return new Promise((resolve, reject) => {
    var column = this.columnManager.findColumn(field);
    if (column) {
      this.columnManager.scrollToColumn(column, position, ifVisible).then(() => {
        resolve();
      }).catch(err => {
        reject(err);
      });
...
_Tabulator.prototype.setPageToRow = function (row) {
  return new Promise((resolve, reject) => {
    if (this.options.pagination && this.modExists("page")) {
      ...
      if (row) {
        this.modules.page.setPageToRow(row).then(() => {
          resolve();
        }).catch(() => {
          reject();
        });
...
Ajax.prototype.nextPage = function (diff) {
  ...
  if (!this.loading) {
    ...
    if (diff < margin) {
      this.table.modules.page.nextPage().then(() => {}).catch(() => {});
    }
  }
};
...
Ajax.prototype._loadDataStandard = function (inPosition, columnsChanged) {
  return new Promise((resolve, reject) => {
    this.sendRequest(inPosition).then(data => {
      this.table.rowManager.setData(data, inPosition, columnsChanged).then(() => {
        resolve();
      }).catch(e => {
        reject(e);
      });
...
Clipboard.prototype.initialize = function () {
  ...
  if (this.mode === "fullTableCopyOnly") {
    this.table.element.addEventListener("copy", e => {
      ...
      console.log("J: Inside fullTableCopyOnly handler:", this.isFullTableCopy);
      if (this.isFullTableCopy) {
        var plain, html, list;
        if (!this.blocked) {
          e.preventDefault();
...
Page.prototype._getRemotePage = function () {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.table.modules.ajax.sendRequest(this.progressiveLoad).then(data => {
...
ResponsiveLayout.prototype.initialize = function () {
  ...
  for (var col of this.table.columnManager.columnsByIndex) {
